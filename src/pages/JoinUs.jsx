...
function JoinUs() {
    return (
        <div id="join-us-section" className="min-h-fit flex flex-col items-center justify-center bg-[#0a0a0a] px-4 py-12">
            ...
            <div className="w-full max-w-5xl rounded-[25px] border border-white/25 backdrop-blur-[18.6px] bg-white/5 flex flex-row flex-wrap items-center justify-center gap-4 md:gap-16 py-8 px-2 md:px-12 shadow-lg">
                {socialLinks.map((item) => (
                    ...
                    >
                        <img
                            src={item.icon}
                            alt={item.type}
                            className="w-12 h-12 md:w-32 md:h-32 object-contain rounded-full shadow-md bg-transparent"
                        />
                    </a>
                ))}
            </div>
